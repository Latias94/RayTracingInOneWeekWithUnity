...
public class Chapter08
{
    ...
    /// <summary>
    /// Rejection method，如果随机的一个点不在圆内，则再重新生成
    /// </summary>
    public static Vector3 RandomInUnitSphere()
    {
        ...
        do
        {
            // 随机在单位正方体中找一点，看其长度是否大于1，大于1则在圆外
            // p = 2.0f * new Vector3(Random.Range(0, 1f), Random.Range(0, 1f), Random.Range(0, 1f)) - Vector3.one;
            p = new Vector3(Random.Range(-1f, 1f), Random.Range(-1f, 1f), Random.Range(-1f, 1f)).normalized;
        } while (p.sqrMagnitude >= 1.0f); // 不直接算Magnitude是因为这样可以少开一次根
